...
function spyropress_row_wrapper( $row_ID, $row ) {
    ...
    if( isset( $row['options']['container_skin'] ) && !empty( $row['options']['container_skin'] ) )
        $section_class[] = $row['options']['container_skin'];

    $parallax = '';
    if( isset( $row['options']['container_skin'] ) && !empty( $row['options']['container_skin'] ) && 'parallax' == $row['options']['container_skin'] )
        $parallax = ' data-stellar-background-ratio="0.5" style="background-image: url(' . $row['options']['parallax_bg'] . ');"';
    ...
    $row_html = sprintf( '
        ...
        </div>', $row_ID, spyropress_clean_cssclass( $section_class ), get_row_class( true ), builder_render_frontend_columns( $row['columns'] ), $parallax
    );
...
function oembed_modifier( $html ) {
    return '<div class="video-responsive">' . preg_replace( '/(width|height|frameborder)="\d*"\s/', "", $html ) . '</div>';
}
