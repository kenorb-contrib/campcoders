...
function spyropress_ui_gallery( $item, $id, $value, $is_widget = false, $is_builder = false ) {
    ...
    echo '<div ' . build_section_class( 'section-gallery section-full', $item ) . '>';
        printf( '<input class="gallery_reset_button button-red button-large pull-right" type="button" value="Clear Gallery" /> ', $id );
        printf( '<input class="gallery_upload_button button button-primary button-large right" type="button" value="Upload" id="upload_%s" />', $id );
        build_heading( $item, $is_widget );
        build_description( $item );
        echo '<div class="controls">';
